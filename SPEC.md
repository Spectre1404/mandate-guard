...
**GET /v1/sessions/{id}/payment-result** — poll every 2s, back off to 5s, and **run until the session's own `expires_at` + 10s grace** (not a fixed wall-clock timeout). Rationale: polling `payment-result` is quota-free (observed Jul 31 — only `POST /v1/sessions` consumes the sandbox allowance), and a poller that gives up before the session does manufactures false failures while the session is still payable. Observed first-time device binding took 160s of a 180s fixed timeout — 20s of headroom, which is not a margin worth defending. The session's `expires_at` is the only real deadline, so it is the one the poller uses:
- Status machine: `pending → processing → awaiting_result → completed | failed`. The docs disagree on whether `processing` appears (`/concepts/checkout-flow.md` documents it — card entry, OTP, passkey in progress; `/api-reference/get-payment-result.md` omits it from the enum), so the poller treats `processing` as a normal transient state and logs-without-erroring on any unknown status. **CONFIRMED OBSERVED (Aug 1 integration checkpoint):** the real sandbox emitted `pending → processing → awaiting_result` during a first-time device enrolment, so `processing` is real and `get-payment-result.md`'s enum is the incomplete one. A poller that treated an unlisted status as fatal would have failed this run.
